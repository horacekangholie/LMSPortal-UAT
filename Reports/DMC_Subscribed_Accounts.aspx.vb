...
Partial Class Reports_DMC_Subscribed_Accounts
    Inherits LMSPortalBaseCode

    Dim PageTitle As String = "DMC Subscription Account Report"
    ...
    Protected Sub BuildContenctPage(Optional ByVal TB_Search As String = "")
        ...
        Try
            Dim sqlStr() As String = {"SELECT * FROM _DMC_All_Subscribed_Accounts WHERE Account_Type = 'Billed' AND (Customer LIKE '%" & keyword & "%' OR [Group] LIKE '%" & keyword & "%' OR HQ_Code LIKE '%" & keyword & "%' OR HQ_Name LIKE '%" & keyword & "%' OR Store_Name LIKE '%" & keyword & "%') ORDER BY Customer, Store_No ",
                                      "SELECT * FROM _DMC_All_Subscribed_Accounts WHERE Account_Type = 'Demo' AND (Customer LIKE '%" & keyword & "%' OR [Group] LIKE '%" & keyword & "%' OR HQ_Code LIKE '%" & keyword & "%' OR HQ_Name LIKE '%" & keyword & "%' OR Store_Name LIKE '%" & keyword & "%') ORDER BY Customer, Store_No ",
                                      "SELECT * FROM _DMC_All_Subscribed_Accounts WHERE Account_Type = 'Trial' AND (Customer LIKE '%" & keyword & "%' OR [Group] LIKE '%" & keyword & "%' OR HQ_Code LIKE '%" & keyword & "%' OR HQ_Name LIKE '%" & keyword & "%' OR Store_Name LIKE '%" & keyword & "%') ORDER BY Customer, Store_No ",
                                      "SELECT * FROM _DMC_All_Subscribed_Accounts WHERE Account_Type = 'Test' AND (Customer LIKE '%" & keyword & "%' OR [Group] LIKE '%" & keyword & "%' OR HQ_Code LIKE '%" & keyword & "%' OR HQ_Name LIKE '%" & keyword & "%' OR Store_Name LIKE '%" & keyword & "%') ORDER BY Customer, Store_No "}

            ' Build and bind Gridview
            BuildGridView(GridView1, "GridView1", "Store_ID")
            GridView1.DataSource = GetDataTable(sqlStr(0))
            GridView1.DataBind()

            BuildGridView(GridView2, "GridView2", "Store_ID")
            GridView2.DataSource = GetDataTable(sqlStr(1))
            GridView2.DataBind()

            BuildGridView(GridView3, "GridView3", "Store_ID")
            GridView3.DataSource = GetDataTable(sqlStr(2))
            GridView3.DataBind()

            BuildGridView(GridView4, "GridView4", "Store_ID")
    ...
    Protected Sub BuildGridView(ByVal ControlObj As Object, ByVal ControlName As String, ByVal DataKeyName As String)
        ...
        Select Case ControlName
            Case "GridView1"
                ...
                Dim ColData() As String = {"Customer", "Group", "HQ_Code", "HQ_Name", "Store_No", "Store_Name", "Created_Date", "Start_Date", "End_Date", "Duration", "Currency", "Fee", "Status", "Account_Type", "Requested_By"}
                Dim ColSize() As Integer = {200, 100, 50, 200, 50, 200, 50, 50, 50, 50, 50, 50, 50, 50, 100}
            ...
            Case "GridView2", "GridView3", "GridView4"
                GridViewObj.Columns.Clear()
                Dim ColData() As String = {"Customer", "Group", "HQ_Code", "HQ_Name", "Store_No", "Store_Name", "Created_Date", "Status", "Account_Type"}
                Dim ColSize() As Integer = {200, 100, 50, 200, 50, 200, 50, 50, 100}
                For i = 0 To ColData.Length - 1
                    Dim Bfield As BoundField = New BoundField()
                    Bfield.HeaderText = Replace(ColData(i), "_", " ")
                    Bfield.HeaderStyle.Width = ColSize(i)
