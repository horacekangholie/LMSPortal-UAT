...
Partial Class Reports_DMC_Users
    Inherits LMSPortalBaseCode

    Dim PageTitle As String = "DMC Users"
    Dim sqlStr As String = "SELECT ROW_NUMBER() OVER(ORDER BY [Headquarter ID] ASC) AS No, * FROM R_DMC_User_Licence "

    Dim reportColName As String() = {"Username", "Password", "Email", "Created Date", "Inactive Date", "Status", "Device Type", "Customer"}
    Dim reportColSize As Integer() = {100, 100, 200, 100, 100, 100, 100, 300}
    Dim orderByCol As String = "[Headquarter ID]"
    ...
    Protected Sub BuildPageContent(Optional ByVal TB_Search As String = "")
        ...
        Try
            sqlStr += " WHERE [Headquarter ID] LIKE '%" & keyword & "%' OR [Headquarter Name] LIKE '%" & keyword & "%' OR [Username] LIKE '%" & keyword & "%' OR [Email] LIKE '%" & keyword & "%' OR [Customer] LIKE '%" & keyword & "%' "
        ...
        ReportSQL.Text = " SELECT ROW_NUMBER() OVER(ORDER BY [Headquarter ID] ASC) AS No, [Username], [Password], [Email], FORMAT([Created Date], 'yyyy-MM-dd') AS [Created Date], FORMAT([Inactive Date], 'yyyy-MM-dd') AS [Inactive Date], [Status], [Device Type], [Customer] " &
                         " FROM R_DMC_User_Licence " &
                         " WHERE [Headquarter ID] LIKE '%" & keyword & "%' OR [Headquarter Name] LIKE '%" & keyword & "%' OR [Username] LIKE '%" & keyword & "%' OR [Email] LIKE '%" & keyword & "%' OR [Customer] LIKE '%" & keyword & "%' " &
                         " ORDER BY " & orderByCol
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            e.Row.Cells(GetColumnIndexByName(e.Row, "Password")).Style.Add("font-family", "Menlo,Monaco,Consolas,Liberation Mono,courier New,monospace")
            e.Row.Cells(GetColumnIndexByName(e.Row, "Password")).Style.Add("background-color", "#fbfcfc")
        End If
    End Sub
    ...
    Protected Sub BT_Download_Excel_Click(ByVal sender As Object, ByVal e As EventArgs) Handles BT_Download_Excel.Click
        DownloadExcel(ReportSQL.Text, "DMC User Account", "Excel", "General")
    End Sub
