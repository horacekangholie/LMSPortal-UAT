...
Partial Class Views_DMC_Account_Revenue
    Inherits LMSPortalBaseCode

    Dim PageTitle As String = "DMC Account Revenue"
    Dim MonthlyTotal, MonthlyAverage, MonthlyMedian, MonthlyNoOfStore As String
    ...
    Protected Sub BuildContenctPage(Optional ByVal ReportMonth As String = Nothing)
        ...
        ReportMonth = IIf(ReportMonth Is Nothing, DateSerial(Year(Now), Month(Now) - 1, 1).ToString("yyyy-MM-dd"), ReportMonth)   '' Return first day of previous of month

        '' Get the MonthlyTotal, MonthlyAverage, MonthlyNoOfStore and MonthlyMedian based on Report Month
        Dim dReader = RunSQLExecuteReader("SELECT SUM(Monthly_Fee) AS Total_Amount, CAST(AVG(Monthly_Fee) AS decimal(10,2)) AS Average, COUNT(Store_No) AS No_Of_Store, dbo.Get_Revenue_Median('" & GetEndOfMonthDate(ReportMonth).ToString("yyyy-MM-dd") & "') AS Median FROM dbo.DMC_Monthly_Subscription('" & GetEndOfMonthDate(ReportMonth).ToString("yyyy-MM-dd") & "') ")
        While dReader.Read()
            MonthlyTotal = String.Format("{0:#,##0.00}", dReader("Total_Amount"))
            MonthlyAverage = String.Format("{0:#,##0.00}", dReader("Average"))
            MonthlyMedian = String.Format("{0:#,##0.00}", dReader("Median"))
            MonthlyNoOfStore = String.Format("{0:0}", dReader("No_Of_Store"))
        End While
        dReader.Close()

        Try
            '' Run store procedured to populate data to Temptable_DMC_Monthly_Revenue_Summary in SQL
            Dim StartMonth As String = DateSerial(Year(Now) - 5, Month(Now), 1).ToString("yyyy-MM-dd")    '' Return first date of that month five year ago
            Dim EndMonth As String = DateSerial(Year(Now), Month(Now), 0).ToString("yyyy-MM-dd")          '' Return end date of that month of previous month

            '' Insert data to temp table
            RunSQL("EXEC dbo.SP_Insert_Temptable_DMC_Monthly_Revenue_Summary '" & StartMonth & "', '" & EndMonth & "' ")

            Dim sqlStr() As String = {"SELECT * FROM dbo.DMC_Monthly_Subscription('" & GetEndOfMonthDate(ReportMonth).ToString("yyyy-MM-dd") & "') ORDER BY Customer, Store_No ",
                                      "SELECt * FROM R_DMC_Subscription_Revenue_Overview ORDER BY [Year] DESC, CASE Col WHEN 'Amount' THEN 1 WHEN 'No of Store' THEN 2 ELSE 3 END "}

            ' Build and bind Gridview
            BuildGridView(GridView1, "GridView1", "Subscription_ID")
    ...
    Protected Sub BuildGridView(ByVal ControlObj As Object, ByVal ControlName As String, ByVal DataKeyName As String)
        ...
        Select Case ControlName
            Case "GridView1"
                ...
                Dim ColData() As String = {"Customer", "Headquarter_ID", "Headquarter_Name", "Store_No", "Store_Name", "Created_Date", "Start_Date", "End_Date", "No_Of_Month", "Remaining_Cycle", "Currency", "Monthly_Fee"}
                For i = 0 To ColData.Length - 1
                    ...
                    End If
                    If Bfield.DataField.Contains("Fee") Then
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        Dim ColName() As String = {"Customer", "HQ_ID", "HQ_Name", "Store<br>No", "Store Name", "Created Date", "Start Date", "End Date", "Period<br>(Month)", "Remaining<br>Cycle", "Base<br>Currency", "Fee"}
        Dim ColSize() As Integer = {100, 50, 100, 50, 100, 0, 0, 0, 0, 0, 0, 100}
        ...
        ElseIf e.Row.RowType = DataControlRowType.Footer Then
            e.Row.Cells(GetColumnIndexByName(e.Row, "Store_Name")).Text = "Total Store: " & MonthlyNoOfStore

            e.Row.Cells(GetColumnIndexByName(e.Row, "Remaining_Cycle")).Text = "Total <br /> Mean(Avg) <br /> Median"
            e.Row.Cells(GetColumnIndexByName(e.Row, "Remaining_Cycle")).Style.Add("text-align", "right !important")
            e.Row.Cells(GetColumnIndexByName(e.Row, "Remaining_Cycle")).Style.Add("font-weight", "bold !important")

            e.Row.Cells(GetColumnIndexByName(e.Row, "Currency")).Text = "SGD <br /> SGD <br /> SGD"
            ...
            e.Row.Cells(GetColumnIndexByName(e.Row, "Monthly_Fee")).Text = MonthlyTotal & "<br />" & MonthlyAverage & "<br />" & MonthlyMedian
            e.Row.Cells(GetColumnIndexByName(e.Row, "Monthly_Fee")).Style.Add("text-align", "right !important")
            e.Row.Cells(GetColumnIndexByName(e.Row, "Monthly_Fee")).Style.Add("font-weight", "bold !important")
        End If
    End Sub
    ...
    Protected Sub DDL_ReportMonth_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDL_ReportMonth.SelectedIndexChanged
        BuildContenctPage(CDate(GetEndOfMonthDate(DDL_ReportMonth.SelectedValue)).ToString("yyyy-MM-dd"))
    End Sub
