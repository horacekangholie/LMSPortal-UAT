...
Partial Class Views_Module_Licence_List
    Inherits LMSPortalBaseCode

    Dim PageTitle As String = "Module Licences"
    Dim sqlStr As String = "SELECT ROW_NUMBER() OVER(ORDER BY Name ASC) AS No, * " &
                           "FROM R_Customer_List " &
                           "WHERE [Customer ID] IN (SELECT DISTINCT Customer_ID FROM LMS_Module_Licence_Order) "
    ...
    Protected Sub BuildPageContent(Optional ByVal TB_Search As String = "")
        ...
        Try
            If Len(keyword) > 0 Then
                sqlStr += " AND ([Name] LIKE '%" & keyword & "%' OR [Code] LIKE '%" & keyword & "%' OR [Customer ID] IN (SELECT TOP 1 Customer_ID FROM LMS_Licence WHERE Application_Type IN ('PC Scale') AND Licence_Code LIKE '%" & keyword & "%') ) "
            End If
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            e.Row.Attributes.Add("OnClick", String.Format("window.location='/Views/Module_Licence_Details.aspx?Customer_ID={0}'", drv("Customer ID")))
        End If
    End Sub
