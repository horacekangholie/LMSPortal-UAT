...
Partial Class Views_Hardkey_Licence_List
    Inherits LMSPortalBaseCode

    Dim PageTitle As String = "Harkey Licences"
    Dim sqlStr As String = "SELECT ROW_NUMBER() OVER(ORDER BY Name ASC) AS No, * " &
                           "FROM R_Customer_List " &
                           "WHERE [Customer ID] IN (SELECT DISTINCT Customer_ID FROM LMS_Hardkey_Licence) "
    ...
    Protected Sub BuildPageContent(Optional ByVal TB_Search As String = "")
        ...
        Try
            sqlStr += " AND ([Name] LIKE '%" & keyword & "%' OR ([Code] LIKE '%" & keyword & "%' AND [Code] IS NOT NULL)) "
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            e.Row.Attributes.Add("OnClick", String.Format("window.location='/Views/Hardkey_Details.aspx?Customer_ID={0}'", drv("Customer ID")))
        End If
    End Sub
