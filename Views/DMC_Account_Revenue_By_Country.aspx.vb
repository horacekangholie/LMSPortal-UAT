...
Partial Class Views_DMC_Account_Revenue_By_Country
    Inherits LMSPortalBaseCode

    Dim PageTitle As String = "DMC Revenue By Country"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Dim StartMonth As String = DateSerial(Year(Now) - 5, Month(Now), 1).ToString("yyyy-MM-dd")               '' One year early to track those contract start at one year earlier
        ...
        If Not IsPostBack Then
            Try
                ...
                DDL_Country.DataSource = GetDataTable(sqlStr)
                DDL_Country.DataTextField = "Country"
                DDL_Country.DataValueField = "Country"
                DDL_Country.DataBind()

                Dim i = DDL_Country.Items.IndexOf(DDL_Country.Items.FindByValue("Singapore"))   '' default to select as singapore
                i = IIf(i < 0, 0, i)
        ...
        BuildContenctPage(Nothing, DDL_Country.SelectedValue)
        LB_Country.Text = DDL_Country.SelectedValue
    End Sub

    Protected Sub BuildContenctPage(Optional ByVal ReportMonth As String = Nothing, Optional ByVal Country As String = Nothing)
        ...
        Dim dReader = RunSQLExecuteReader("SELECT COUNT(Headquarter_ID) AS Headquarter_Count, SUM(Owned_Store) AS Store_Count, SUM(Total_Amount_Per_Month) AS Total_Amount FROM dbo.DMC_Monthly_Subscription_By_Country('" & GetEndOfMonthDate(ReportMonth).ToString("yyyy-MM-dd") & "') ")
        While dReader.Read()
        ...
        Try
            ...
            RunSQL("EXEC dbo.SP_Insert_TempTable_DMC_Monthly_Revenue_By_Country_Summary '" & StartMonth & "', '" & EndMonth & "', '" & Country & "' ")

            Dim sqlStr() As String = {"SELECT * FROM dbo.DMC_Monthly_Subscription_By_Country('" & GetEndOfMonthDate(ReportMonth).ToString("yyyy-MM-dd") & "') ",
                                      "SELECT * FROM R_DMC_Subscription_Revenue_By_Country_Overview ORDER BY [Year] DESC, CASE Col WHEN 'Amount' THEN 1 WHEN 'No of Store' THEN 2 ELSE 3 END "}
    ...
    Protected Sub BuildGridView(ByVal ControlObj As Object, ByVal ControlName As String, ByVal DataKeyName As String)
        ...
        Select Case ControlName
            Case "GridView1"
                ...
                Dim ColData() As String = {"Country", "Headquarter_ID", "Headquarter_Name", "Owned_Store", "Currency", "Total_Amount_Per_Month"}
                For i = 0 To ColData.Length - 1
                    Dim Bfield As BoundField = New BoundField()
                    Bfield.DataField = ColData(i)
                    If Bfield.DataField.Contains("Fee") Or Bfield.DataField.Contains("Amount") Then
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        Dim ColName() As String = {"Country", "Headquarter ID", "Headquarter Name", "Owned Store", "Currency", "Total Amount<br>(Monthly)"}
        Dim ColSize() As Integer = {100, 50, 500, 0, 0, 0}
    ...
    Protected Sub DDL_ReportMonth_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDL_ReportMonth.SelectedIndexChanged
        BuildContenctPage(CDate(GetEndOfMonthDate(DDL_ReportMonth.SelectedValue)).ToString("yyyy-MM-dd"), DDL_Country.SelectedValue)
    End Sub
