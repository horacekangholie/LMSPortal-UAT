...
Partial Class Views_Overseas_Customers
    Inherits LMSPortalBaseCode

    Dim PageTitle As String = "Overseas Customers"
    Dim sqlStr As String = "SELECT ROW_NUMBER() OVER(ORDER BY Name ASC) AS No, * " &
                           "FROM R_Customer_List " &
                           "WHERE [Group Name] = 'Overseas Customers' " &
                           "  AND [Customer ID] IN (SELECT Customer_ID FROM DMC_Headquarter) "
    ...
    Protected Sub BuildPageContent(Optional ByVal TB_Search As String = "")
        ...
        Try
            sqlStr += " AND ([Name] LIKE '%" & keyword & "%' OR [By Distributor] LIKE '%" & keyword & "%' OR [Country] LIKE '%" & keyword & "%' OR [Headquarter ID] LIKE '%" & keyword & "%') "
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            e.Row.Attributes.Add("OnClick", String.Format("window.location='/Views/Subscription_Details.aspx?Customer_ID={0}&Group={1}'", drv("Customer ID"), "4"))
        End If
    End Sub
