...
Partial Class Form_App_Product_Licence
    ...
    Protected Sub BuildPageContent(Optional ByVal TB_Search As String = "")
        ...
        Try
            sqlStr += " WHERE [Name] LIKE '%" & keyword & "%' OR [Country] LIKE '%" & keyword & "%' OR ([Code] LIKE '%" & keyword & "%' AND [Code] IS NOT NULL) OR [Customer ID] IN (SELECT TOP 1 Customer_ID FROM LMS_Licence WHERE Application_Type NOT IN ('PC Scale') AND Licence_Code LIKE '%" & keyword & "%') "
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            EditLinkButton.PostBackUrl = "~/Form/App_Product_Licence_Form.aspx?Customer_ID=" & drv("Customer ID")
        End If
    End Sub
